...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        if (systemUser == null) {
            //用户不存在
            throw new AuthenticationException(String.valueOf(AuthError.USER_NO_EXIT));
        } else if (systemUser.getStatus()==0) {
            //帐号停用
            throw new AuthenticationException(String.valueOf(AuthError.USER_STOP));
        } else if(!passWord.equals(systemUser.getPassword())){
            //密码错误
            throw new AuthenticationException(String.valueOf(AuthError.USER_ERROR_PASSWORD));
        }
        //登录成功
