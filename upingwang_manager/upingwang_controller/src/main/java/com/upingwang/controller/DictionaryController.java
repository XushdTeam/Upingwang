...
public class DictionaryController extends GlobalController {
     ...
     * @return
     */
    @SystemControllerLog(module = "字典管理",methods = "字典列表")
    @RequestMapping(value="/dictionary/list",method = RequestMethod.POST)
    @ResponseBody
    public JsonResult dictionaryList(){
        List<SystemDictionary> list = dicService.getDicList(false);
    ...
    public String handle(@PathVariable String id, Model model){
        ...
        List<SystemDictionary> list = dicService.getDicList(true);
        if(StringUtils.equals("null",id)){
            ...
            //修改
            SystemDictionary dic = dicService.getDicById(Long.valueOf(id));
    ...
    @ResponseBody
    public JsonResult dictionarySave(@Valid SystemDictionary dictionary, BindingResult result){
        //验证有错误
        if(result.hasErrors()){
            String error = "";
    ...
    @ResponseBody
    public JsonResult dictionaryUpdate(@Valid SystemDictionary dictionary, BindingResult result){
        //验证有错误
        if(result.hasErrors()){
            String error = "";
            List<ObjectError> allErrors = result.getAllErrors();
